...
p.append(sh.occ.addPoint(r,-L1-1.5*R,0)) # 0
p.append(sh.occ.addPoint(r+e,-L1-1.5*R,0)) # 1 
p.append(sh.occ.addPoint(r+e,-1.5*R,0)) #2
p.append(sh.occ.addPoint(r,-1.5*R,0)) #3




p.append(sh.occ.addPoint(r,+1.5*R,0)) #4
p.append(sh.occ.addPoint(r+e,+1.5*R,0)) #5
p.append(sh.occ.addPoint(r+e,+L2+1.5*R,0)) #6 
p.append(sh.occ.addPoint(r,+L2+1.5*R,0)) #7
...
for i in range(-n+1,n):
    y = i*R*1.5/n
    x = e+r+(R-r)*math.cos(math.pi*i/n/2)
    p.append(sh.occ.addPoint(x,y,0)) # 3 Centre

for i in range(n-1,-n,-1):
    y = i*R*1.5/n
    x = r+(R-r)*math.cos(math.pi*i/n/2)
    p.append(sh.occ.addPoint(x,y,0)) # 3 Centre

...
def getMetafor(parm):
    ...
    materset(1).put(w.ELASTIC_MODULUS,2e6) #750e3
    materset(1).put(w.MASS_DENSITY,1060)
    materset(1).put(w.POISSON_RATIO,0.3)

    #materset.define(2,w.ElastHypoMaterial)
    #materset(2).put(w.ELASTIC_MODULUS,2e6)#750e3
    #materset(2).put(w.MASS_DENSITY,1060)
    #materset(2).put(w.POISSON_RATIO,0.3)
    ...
    prp2 = w.ElementProperties(w.NodStress2DElement) # Contraintes nodales
    ...
    materset(2).put(w.PROF_CONT,0.0005)      # 0.001?
